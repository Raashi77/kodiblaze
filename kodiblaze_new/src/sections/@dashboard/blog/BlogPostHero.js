...
export default function BlogPostHero({ post }) {
  const { image, title, name, profilePic, blog_timeStamp } = post;
    ...
    <Box sx={{ position: 'relative' }}>
      ...
      <FooterStyle>
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <Avatar alt={name} src={imageBaseUrl+profilePic} sx={{ width: 48, height: 48 }} />
          <Box sx={{ ml: 2 }}>
      ...
      <OverlayStyle />
      <Image alt="post cover" src={imageBaseUrl+image} ratio="16/9" />
    </Box>
  );
}
