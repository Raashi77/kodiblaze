...
export default function BlogPostCard({ post, index }) {
  const { image, title, view, comment, share, name,profilePic, blog_timeStamp,blog_slug } = post;
  const linkTo = `viewblog/${blog_slug}`;
    ...
    <Grid item xs={12} sm={latestPostLarge ? 12 : 6} md={latestPostLarge ? 6 : 3}>
      <Card sx={{ position: 'relative' }}>
        ...
        >
          ...
          <Image alt={title} src={imageBaseUrl+image} sx={{ position: 'absolute', top: 0, bottom: 0, left: 0, right: 0 }} />
        </Box>
